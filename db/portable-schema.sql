...
-- Offline document leases (Supabase migration 0017), using the portable identity adapter.
...
create policy offline_document_leases_select on public.offline_document_leases
  ...
  using (
    exists (
      select 1 from public.app_users viewer
      where viewer.id = app.current_user_id() and viewer.deactivated_at is null
    )
    and (
      user_id = app.current_user_id()
      or public.can_manage_document(document_id, app.current_user_id())
...
begin
  ...
  if not found or not coalesce(permitted, false)
     or not public.can_read_document(target_document_id, me) then
...
begin
  ...
  loop
    ...
    if not active_profile or not found or not coalesce(permitted, false)
       or not public.can_read_document(checked_id, me) then
      update public.offline_document_leases l
      set revoked_at = now(),
...
revoke all on table public.offline_document_leases
  from public, app_user;
grant select on table public.offline_document_leases to app_user;
